  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <aside className="flex w-64 flex-col border-r border-[var(--color-border)] bg-white">
        <div className="px-6 py-6 text-lg font-bold tracking-tight text-[var(--color-ink)]">System erp</div>
        ...
        <div className="p-4">
          <button onClick={doLogout} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold text-[var(--color-ink-secondary)] transition hover:bg-black/5">
      ...
      <main className="flex-1 overflow-auto bg-[var(--color-surface)]">
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-[var(--color-border)] bg-white/80 px-8 py-4 backdrop-blur">
          ...
          <div className="text-sm font-medium text-[var(--color-ink)]">{user?.email}</div>
        </header>
        <div className="p-6">
          <Outlet />
        </div>
